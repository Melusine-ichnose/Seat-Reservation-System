<template>
  <div class="area-selection-container">
    ...
    <!-- 底部导航栏 -->
    <TabLayout>
      <div @click="navigateTo(tab.url)"
           :style="isActive(tab.url)?'color:#409eff;font-weight:bold;':''"
...
export default {
  ...
  data() {
    return {
      ...
      username: '',
      tabs: [
        {name: '🏠 区域选择', url: '/teacher/area-selection'},
  ...
  created() {
    ...
    if (user) {
      this.username = user.username || '教师';
    }
    this.loadAreas();
  }
}
