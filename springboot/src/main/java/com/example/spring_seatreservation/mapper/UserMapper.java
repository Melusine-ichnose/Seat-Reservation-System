...
public interface UserMapper {
    ...
    // 获取学生当前所有活跃预约（包括待签到、使用中、暂离）
    @Select("SELECT * FROM reservation WHERE uid=${uid} AND state IN (0, 1, 3)")
    List<Map<String, Object>> getActiveReservation(Object uid);
    ...
    @Update("update reservation set state=5 where rid=${rid}")
    void cancelReservation(Object rid);
}
